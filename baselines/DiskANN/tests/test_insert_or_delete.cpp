...
void run_workload(const Config &cfg, diskann::Distance<T> *dist_cmp) {
  ...
  // after each round and score against that round's groundtruth. The active
  // set changes every round, so each round has its own GT file
  // <gt_dir>/round_<k>_gt<recall_at>.bin (generated by the prepare step via
  // incremental_groundtruth). Tags from search_sync are original row IDs,
  // directly comparable to the GT id/tag space.
  T *rq_query = nullptr;
