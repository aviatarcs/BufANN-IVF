...
//  reader.reset(new diskann::MemAlignedFileReader());
  ...
  // Pin each OpenMP worker to a distinct logical CPU (tid -> CPU tid % ncpu) so
  // threads don't migrate. Done in-code because OMP_PLACES/OMP_PROC_BIND and
  // KMP_AFFINITY are unusable here: the binary links two OpenMP runtimes
  // (libgomp + libomp via MKL), which swallow or assert on those env vars.
  // libgomp reuses this thread pool for the timed search loop below, so the
  // binding carries over. Run with all OMP_*/KMP_* affinity vars unset.
  // Gated on DISKANN_PIN so the same binary serves both the pinned and the
  // unpinned (scheduler-managed) arm of the A/B comparison.
  const char* pin_env = std::getenv("DISKANN_PIN");
  if (pin_env != nullptr && std::atoi(pin_env) != 0) {
...
#pragma omp parallel
    {
      ...
      cpu_set_t set;
      CPU_ZERO(&set);
      CPU_SET((int) (tid % ncpu), &set);
      sched_setaffinity(0, sizeof(set), &set);
    }
    diskann::cout << "Thread pinning: ON (tid % " << ncpu << ")" << std::endl;
  } else {
    diskann::cout << "Thread pinning: OFF" << std::endl;
  }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    float mean_expanded = (float) diskann::get_mean_stats(
        stats.data(), query_num,
        [](const diskann::QueryStats& stats) { return stats.n_expanded; });
    float recall = 0;
    if (calc_recall_flag) {
    ...
    std::cout << "{";
    ann_bench::json_kv(std::cout, first, "baseline", "DiskANN");
    ...
    ann_bench::json_kv(std::cout, first, "mean_hops", static_cast<double>(mean_hops));
    ann_bench::json_kv(std::cout, first, "mean_expanded", static_cast<double>(mean_expanded));
