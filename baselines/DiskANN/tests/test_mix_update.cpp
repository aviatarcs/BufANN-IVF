...
struct Config {
  ...
  uint32_t   beamwidth;
  uint32_t   merge_maxc;
  uint32_t   skip_update_search;       // 0/1
  uint32_t   build_only;               // 0/1
...
Config parse_args(int argc, char **argv) {
  if (argc < 35) {
    std::cerr
        ...
        << " <skip_update_search[0/1]> <build_only[0/1]>"
  ...
  cfg.build_only = parse_u32(argv[arg_no++], "build_only", true);
  ...
  if (cfg.skip_update_search > 1 || cfg.build_only > 1 ||
      cfg.enable_interval_metrics > 1) {
    throw std::invalid_argument(
        "skip_update_search, build_only, and enable_interval_metrics flags "
        "must be 0 or 1");
  }
...
void seed_iter(const std::string &inserted_points_file,
  ...
  std::vector<uint32_t> delete_vec =
      take_scripted_ids(::scripted_delete_ids, ::scripted_delete_offset,
  ...
  std::vector<uint32_t> insert_vec =
      take_scripted_ids(::scripted_insert_ids, ::scripted_insert_offset,
...
void merge_kernel(diskann::MergeInsert<T> &merge_insert) {
  ...
  ::maintenance_phase_time_s +=
      std::chrono::duration<double>(merge_end - merge_begin).count();
}

template<typename T, typename TagT = uint32_t>
void run_single_iter(const Config &cfg,
                     size_t iter,
                     diskann::MergeInsert<T> & merge_insert,
                     const std::string &,
                     const std::string &,
  ...
  if (::phase_window_metrics != nullptr) {
    ...
    ann_bench::emit_phase_checkpoint(
        std::cout, "DiskANN", "foreground", iter,
  ...
  if (::phase_window_metrics != nullptr) {
    ...
    if (post_merge_query_count > 0) {
      ...
      ann_bench::emit_phase_checkpoint(
          std::cout, "DiskANN", "post_merge_query", iter,
...
#pragma omp parallel for num_threads((int) cfg.pool_threads) schedule(dynamic, 1)
  ...
  ann_bench::json_kv(std::cout, rfirst, "baseline", "DiskANN");
...
void run_all_iters(const Config &cfg, diskann::Distance<T> *dist_cmp,
  ...
  if (::phase_window_metrics != nullptr && query_pool != nullptr) {
    ::phase_window_metrics->start(
        "DiskANN", ann_bench::latency_reserve_hint(
                       static_cast<uint64_t>(query_pool->query_n)));
  }
  for (size_t i = 0; i < n_iters; i++) {
    std::cout << "ITER : " << i << std::endl;
    run_single_iter<T, TagT>(cfg, i, merge_insert, base_prefix, merge_prefix,
                             mem_prefix, query_pool,
                             dist_cmp);
  }
}
...
void run_workload(const Config &cfg, diskann::Distance<T> *dist_cmp) {
  ...
  ::scripted_insert_offset = 0;
  ::scripted_delete_offset = 0;
  ...
  ::scripted_insert_ids = load_ids_file(cfg.insert_ids_file);
  ::scripted_delete_ids = load_ids_file(cfg.delete_ids_file);
