...
void gen_random_slice(const std::string data_file, double p_val,
                      ...
                      size_t npts_override) {
  size_t   npts;
  uint32_t npts32, ndims32;

  int fd = open(data_file.c_str(), O_RDONLY);
  if (fd < 0)
    throw diskann::ANNException("gen_random_slice: cannot open " + data_file,
                                -1);
  if (::pread(fd, &npts32, sizeof(uint32_t), 0) != sizeof(uint32_t) ||
      ::pread(fd, &ndims32, sizeof(uint32_t), sizeof(uint32_t)) !=
          sizeof(uint32_t)) {
    close(fd);
    throw diskann::ANNException("gen_random_slice: cannot read header", -1);
  }
  npts = npts32;
  ndims = ndims32;
  ...
  const size_t row_bytes = ndims * sizeof(T);
  const off_t  data_start = 2 * sizeof(uint32_t);
  ...
  sampled_ids.reserve(static_cast<size_t>(npts * p_val * 1.1));
  for (size_t i = 0; i < npts; i++) {
    if (distribution(generator) < (float) p_val)
      sampled_ids.push_back(i);
  }
  if (sampled_ids.empty())
    sampled_ids.push_back(0);

  slice_size = sampled_ids.size();
  sampled_data = new float[slice_size * ndims];

  // Parallel random reads: pread() is thread-safe on a shared fd (explicit
  // offset, no shared seek position), and each thread writes a disjoint slice
  // of sampled_data. This saturates NVMe queue depth instead of issuing one
  // serial ~400B read at a time (~100 MB/s -> device-bound).
  bool read_ok = true;  // only ever set false; benign data race
#pragma omp parallel
  {
    std::vector<T> row_buf(ndims);
#pragma omp for schedule(static)
    for (long long s = 0; s < static_cast<long long>(slice_size); s++) {
      off_t offset =
          data_start + static_cast<off_t>(sampled_ids[s]) * row_bytes;
      ssize_t got = ::pread(fd, row_buf.data(), row_bytes, offset);
      if (got != static_cast<ssize_t>(row_bytes)) {
        read_ok = false;
        continue;
      }
      for (size_t d = 0; d < ndims; d++)
        sampled_data[static_cast<size_t>(s) * ndims + d] =
            static_cast<float>(row_buf[d]);
    }
  }
  close(fd);
  if (!read_ok)
    throw diskann::ANNException("gen_random_slice: short pread", -1);

  diskann::cout << "gen_random_slice: sampled " << slice_size << " / " << npts
                << " rows via pread (read "
                << (slice_size * row_bytes) / (1024 * 1024) << " MB instead of "
                << (npts * row_bytes) / (1024 * 1024) << " MB)" << std::endl;
}
...
int partition_with_ram_budget(const std::string data_file,
  ...
  while (!fit_in_ram) {
    ...
    estimate_cluster_sizes<T>(data_file, pivot_data, num_parts, train_dim,
                              k_base, cluster_sizes, npts_override);
  ...
  shard_data_into_clusters<T>(data_file, pivot_data, num_parts, train_dim,
                              k_base, prefix_path, npts_override);
  delete[] pivot_data;
  delete[] train_data_float;
  return num_parts;
}
...
template void DISKANN_DLLEXPORT gen_random_slice<float>(
    ...
    size_t &slice_size, size_t &ndims, size_t npts_override);
template void DISKANN_DLLEXPORT gen_random_slice<uint8_t>(
    const std::string data_file, double p_val, float *&sampled_data,
    size_t &slice_size, size_t &ndims, size_t npts_override);
template void DISKANN_DLLEXPORT gen_random_slice<int8_t>(
