...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void create_disk_layout(const std::string &mem_index_file,
    ...
    npts_64 = npts;
    ndims_64 = ndims;
    if (npts_override > 0 && npts_override < npts_64) {
      diskann::cout << "create_disk_layout: clamping npts from " << npts_64
                    << " to npts_override=" << npts_override << std::endl;
      npts_64 = npts_override;
      npts = (unsigned) npts_override;
    }
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath,
    ...
    if (std::getenv("SKIP_PQ") == nullptr) {
      ...
      diskann::cout << "SKIP_PQ=1 set — skipping PQ pivot training and "
                       "compression. _pq_pivots.bin and _pq_compressed.bin "
                       "will NOT be produced. Generate them separately before "
                       "running search."
                    << std::endl;
    }
    start = std::chrono::high_resolution_clock::now();
  ...
  template DISKANN_DLLEXPORT void create_disk_layout<int8_t, uint32_t>(
      ...
      const std::string &output_file, size_t npts_override);
