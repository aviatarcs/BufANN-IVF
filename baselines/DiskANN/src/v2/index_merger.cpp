...
namespace diskann {
  ...
  StreamingMerger<T, TagT>::StreamingMerger(
      ...
      const uint32_t maxc, bool single_file_index, uint64_t merge_nthreads) {
    // book keeping
    this->ndims = ndims;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads((int) this->merge_nthreads)
    ...
    if (n_inserts > 0) {
      ...
      diskann::cout << "TIMER:: PQ time per point = 0, Delta = 0\n";
    }
  ...
  void StreamingMerger<T, TagT>::process_inserts() {
    ...
    const uint32_t insert_threads =
        std::max<uint32_t>(1, std::min<uint32_t>(
                                static_cast<uint32_t>(this->merge_nthreads),
                                static_cast<uint32_t>(this->disk_thread_data.size())));
    this->insert_times.resize(insert_threads, 0.0);
    this->delta_times.resize(insert_threads, 0.0);
    // iterate through each vector in each mem index
    for (uint32_t i = 0; i < this->mem_data.size(); i++) {
...
#endif
      diskann::Timer timer;
#pragma omp parallel for schedule(dynamic, 1) num_threads(insert_threads)
      // iteratively insert each point into full index
      for (int32_t j = 0; j < (int32_t) count; j++) {
        // filter out -- `j` is deleted
        if (deleted_set.find((uint32_t) j) != deleted_set.end()) {
    ...
    if (n_inserts > 0) {
      ...
      diskann::cout << "TIMER:: Insert time per point = 0, Delta = 0\n";
    }
...
#pragma omp parallel for schedule(dynamic, 128) num_threads((int) this->merge_nthreads)
...
#pragma omp parallel for schedule(dynamic, 128) num_threads((int) this->merge_nthreads)
  ...
  void StreamingMerger<T, TagT>::merge(const char *                    disk_in,
    ...
    // A fresh StreamingMerger is built per merge round (MergeInsert::switch_index
    // -> construct_index_merger), and getTempFilePath() increments temp<i> while
    // the prior file still exists -- so temp indexes accumulate ~1 full
    // disk.index per merge and fill the disk at billion scale (ENOSPC). After
    // mergeImpl the only temp still needed is the one just made live
    // (this->temp_*); drop every other temp*_temp_* in the temp dir so disk
    // usage stays ~2x (ping-pong). See OVERNIGHT_NOTES.md.
    if (this->temp_disk_index_path.find("_temp_disk_index") != std::string::npos) {
      auto base_of = [](const std::string &p) {
        auto s = p.find_last_of('/');
        return s == std::string::npos ? p : p.substr(s + 1);
      };
      const std::string slash = this->temp_disk_index_path;
      auto sp = slash.find_last_of('/');
  ...
  void StreamingMerger<T, TagT>::mergeImpl() {
    ...
    // alternative using list; delete-only merge can have no inserts.
    ...
    const bool has_live_inserts = !this->inverse_list.empty();
    ...
    auto move_file = [&copy_file](const std::string &src,
                                  const std::string &dest) {
      diskann::cout << "MOVE :: " << src << " --> " << dest << "\n";
    ...
    if (has_live_inserts) {
      ...
      const std::function<uint32_t(uint32_t)> rename_func =
          [this](uint32_t id) { return this->rename(id); };
      ...
      diskann::cout << "No live inserts in this merge; using delete "
                    << "consolidated graph as final output.\n";
      assert(this->disk_npts == new_max_pts);
      move_file(this->temp_disk_index_path, this->final_index_file);
    }
