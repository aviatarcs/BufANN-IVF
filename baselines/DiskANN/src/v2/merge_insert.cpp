...
namespace diskann {
  ...
  MergeInsert<T, TagT>::MergeInsert(
    ...
    _paras_mem.Set<bool>("saturate_graph",
                         parameters.Get<bool>("saturate_mem", false));
    ...
    _paras_disk.Set<bool>("saturate_graph",
                          parameters.Get<bool>("saturate_disk", false));
 ...
 void MergeInsert<T,TagT>::construct_index_merger()
 {
    ...
    _merger = new diskann::StreamingMerger<T, TagT>((uint32_t) _dim, _dist_comp, _dist_metric, (uint32_t) _beamwidth, range, l_index, alpha, maxc, _single_file_index, _merge_nthreads);
 ...
 template<typename T, typename TagT>
 size_t MergeInsert<T,TagT>::search_disk_only(const T* query, const uint64_t K,
                     const uint64_t search_L, TagT* tags, float * distances,
                     QueryStats * stats)
 {
        std::shared_lock<RWlock> lock(_disk_lock);
        assert(_switching_disk_prefixes == false);
        return _disk_index->cached_beam_search(
            query, K, search_L, tags, distances, _beamwidth, stats);
 }

 template<typename T, typename TagT>
 void MergeInsert<T,TagT>::get_active_tags(tsl::robin_set<TagT>& active_tags)
 {
        std::shared_lock<RWlock> lock(_disk_lock);
        assert(_switching_disk_prefixes == false);
...
//          best_vec.erase(best_vec.begin() + K, best_vec.end());
 ...
 void MergeInsert<T,TagT>::final_merge()
 {
     ...
     size_t mem_points_snapshot = 0;
     {
         std::shared_lock<std::shared_timed_mutex> lock(_change_lock);
         mem_points_snapshot = _mem_points;
     }

     size_t pending_deletes = 0;
     {
         std::shared_lock<std::shared_timed_mutex> lock(_delete_lock);
         pending_deletes = _deletion_set_0.size() + _deletion_set_1.size();
     }

     if ((mem_points_snapshot > 0) || (pending_deletes > 0))
     {
         if ((mem_points_snapshot == 0) && (pending_deletes > 0))
         {
             diskann::cout << "Final merge triggered by pending deletes: "
                           << pending_deletes << std::endl;
         }
         save_del_set();
         switch_index();
     }
