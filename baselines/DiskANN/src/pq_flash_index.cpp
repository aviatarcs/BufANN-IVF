...
// read index metadata
    ...
    for (uint32_t i = 0; i < l_search && res_count < k_search; i++) {
      ...
      if (res_tags != nullptr) {
        if (this->tags != nullptr) {
          ...
          // No tag table: fall back to raw node id so callers without tags
          // still receive meaningful neighbor identifiers.
          res_tags[res_count] = (TagT) expanded_nodes_info[i].id;
        }
      }
      res_count++;
    }
  ...
  size_t PQFlashIndex<T, TagT>::cached_beam_search(
    ...
    for (uint32_t i = 0; i < l_search && res_count < k_search; i++) {
      ...
      if (res_tags != nullptr) {
        res_tags[res_count] = (this->tags != nullptr)
                                  ? this->tags[expanded_nodes_info[i].id]
                                  : (TagT) expanded_nodes_info[i].id;
      }
      res_count++;
    }
    return res_count;
