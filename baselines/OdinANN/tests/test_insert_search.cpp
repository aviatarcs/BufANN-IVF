...
static int get_insert_threads() {
  return static_cast<int>(pipeann::env_u32("INSERT_THREADS", NUM_INSERT_THREADS));
}
...
std::string GetTruthFileName(const std::string &truthFilePrefix, int l_start, int recall_at) {
  std::string fileName = truthFilePrefix + "/gt_" + std::to_string(l_start) + ".bin";
  if (!file_exists(fileName)) {
    fileName = truthFilePrefix + "/sift_" + std::to_string(l_start) + "_gt" + std::to_string(recall_at) + ".bin";
  }
...
void update(const std::string &data_bin, const unsigned L_disk, int vecs_per_step, int num_steps,
  ...
  paras.set(0, L_disk, pipeann::env_u32("C", 384), pipeann::env_f32("ALPHA", 1.2f),
            static_cast<unsigned>(get_search_threads() + get_insert_threads()), true, beam_width);
  ...
  std::string currentFileName = GetTruthFileName(truthset_file, res + truthset_l_offset, recall_at);
  ...
  for (int i = 0; i < num_steps; i++) {
    ...
    res += vecs_per_step;
    currentFileName = GetTruthFileName(truthset_file, res + truthset_l_offset, recall_at);
...
int main(int argc, char **argv) {
  if (argc < 19) {
    LOG(INFO) << "Correct usage: " << argv[0] << " <type[int8/uint8/float]> <data_bin> <L_disk>"
              << " <vecs_per_step> <num_steps> <insert_threads> <merge_threads> <search_threads> <search_mode>"
