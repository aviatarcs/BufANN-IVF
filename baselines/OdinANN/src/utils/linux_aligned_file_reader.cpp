  ...
  void execute_io(void *ctx, int fd, std::vector<IORequest> &reqs, uint64_t n_retries = 0, bool write = false) {
    ...
    for (uint64_t iter = 0; iter < n_iters; iter++) {
      ...
      for (uint64_t j = 0; j < n_ops; j++) {
        const auto &req = reqs[j + iter * MAX_EVENTS];
        if (write) {
          io_prep_pwrite(cb.data() + j, fd, req.buf, req.len, req.offset);
        } else {
...
IOStats LinuxAlignedFileReader::read_alloc(std::vector<IORequest> &read_reqs, void *ctx,
