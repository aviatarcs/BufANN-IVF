...
namespace pipeann {
  ...
  int SSDIndex<T, TagT>::insert_in_place(const T *point1, const TagT &tag, tsl::robin_set<uint32_t> *deletion_set) {
    ...
    T *coord_buf = nullptr;
    alloc_aligned((void **) &coord_buf, 10 * this->params.L * this->aligned_dim * sizeof(T), 256);
    ...
    const IOStats disk_reads = reader->read_alloc(reads, ctx, &page_ref);
    ann_bench::LogicalIoCounter::account_random_read(disk_reads.n_ops, disk_reads.n_bytes);
...
#else
    const IOStats disk_writes = count_io_requests(writes);
    reader->write(writes, ctx);
    ann_bench::LogicalIoCounter::account_random_write(disk_writes.n_ops, disk_writes.n_bytes);
  ...
  void SSDIndex<T, TagT>::bg_io_thread() {
    ...
    while (true) {
      ...
      const IOStats disk_writes = count_io_requests(task->writes);
