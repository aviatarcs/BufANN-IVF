...
namespace pipeann {
  ...
  DynamicSSDIndex<T, TagT>::DynamicSSDIndex(IndexBuildParameters &parameters, const std::string disk_prefix_in,
                                            ...
                                            const std::string &mem_index_prefix_in, bool copy_shadow) {
    const std::string resolved_mem_prefix =
        mem_index_prefix_in.empty() ? disk_prefix_in : mem_index_prefix_in;
    // check if file exists.
    if (!file_exists(disk_prefix_in + "_disk.index")) {
      LOG(ERROR) << "Disk index file does not exist: " << disk_prefix_in << "_disk.index";
      exit(-1);
    }
    if (use_mem_index && !file_exists(resolved_mem_prefix + "_mem.index")) {
      LOG(ERROR) << "In-memory index file does not exist: " << resolved_mem_prefix << "_mem.index";
      exit(-1);
    }
    ...
    if (use_mem_index) {
      std::string mem_index_path = resolved_mem_prefix + "_mem.index";
