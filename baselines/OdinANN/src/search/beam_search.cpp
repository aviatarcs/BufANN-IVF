  ...
  void SSDIndex<T, TagT>::do_beam_search(const T *query1, uint32_t mem_L, uint32_t l_search, const uint32_t beam_width,
    ...
    auto compute_and_add_to_retset = [&](const unsigned *node_ids, const uint64_t n_ids) {
      ...
      for (uint64_t i = 0; i < n_ids; ++i) {
        Neighbor seed(node_ids[i], dist_scratch[i], true);
        if (cur_list_size == 0) {
          retset[0] = seed;
          cur_list_size = 1;
        } else {
    ...
    if (mem_L) {
      ...
      compute_and_add_to_retset(entry_points_.data(), entry_points_.size());
    }
    ...
    while (k < cur_list_size) {
      ...
      if (!frontier.empty()) {
        ...
        io_timer.reset();
        const IOStats disk_reads = reader->read_alloc(frontier_read_reqs, ctx, &page_ref);
