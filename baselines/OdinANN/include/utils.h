...
namespace pipeann {
  ...
  inline void load_bin_impl(std::basic_istream<char> &reader, T *&data, size_t &npts, size_t &dim,
    ...
    data = new T[npts * dim];
    const size_t payload_bytes = npts * dim * sizeof(T);
    reader.read((char *) data, payload_bytes);
  }
  ...
  inline void load_bin_impl(std::basic_istream<char> &reader, std::vector<T> &data, size_t &npts, size_t &dim,
    ...
    const size_t payload_bytes = npts * dim * sizeof(T);
    reader.read((char *) (data.data()), payload_bytes);
  }
    ...
    if (truthset_type == 3) {
      if (tags != nullptr) {
        *tags = new uint32_t[npts * dim];
        ...
        reader.seekg(npts * dim * sizeof(uint32_t), reader.cur);
      }
    }
  }
