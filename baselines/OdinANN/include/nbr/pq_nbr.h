...
#pragma omp parallel for num_threads(nthreads)
    ...
    void insert(T *point, uint32_t loc) {
      ...
      uint64_t pq_offset = loc * pq_table.n_chunks;
      {
        ...
        if (this->data.size() < pq_offset + pq_table.n_chunks) {
          LOG(INFO) << "PQNeighbor::insert(): growing this->data from " << this->data.size() << " to " << 1.5 * (pq_offset + pq_table.n_chunks);
    ...
    void reserve_points(uint64_t point_capacity) override {
      const uint64_t required = point_capacity * pq_table.n_chunks;
      pq_mu.lock();
      LOG(INFO) << "PQNeighbor::reserve_points(): point_capacity = " << point_capacity << " requested.";
      LOG(INFO) << "PQNeighbor::reserve_points(): current data.size()     = " << this->data.size();
      LOG(INFO) << "PQNeighbor::reserve_points(): new minimum data.size() = " << required;
      if (this->data.size() < required) {
        LOG(INFO) << "PQNeighbor::reserve_points(): starting resize data to " << required;
        this->data.resize(required);
        LOG(INFO) << "PQNeighbor::reserve_points(): finished resize data to " << required;
      }
