   ...
   public:
    ...
    void load_pq_centroid_bin(const char *pq_table_file, size_t num_chunks, size_t offset = 0) {
      ...
      if (file_exists(tables_t_cache_path) && file_exists(all_to_all_cache_path)) {
        std::vector<float> cached_tables_t;
        std::vector<float> cached_all_to_all;
        load_bin<float>(tables_t_cache_path, cached_tables_t, nr, nc);
        if (nr == ndims && nc == NUM_PQ_CENTROIDS) {
          load_bin<float>(all_to_all_cache_path, cached_all_to_all, nr, nc);
          if (nr == NUM_PQ_CENTROIDS && nc == NUM_PQ_CENTROIDS * n_chunks) {
            pipeann::alloc_aligned((void **) &tables_T, cached_tables_t.size() * sizeof(float), 64);
            std::memcpy(tables_T, cached_tables_t.data(),
                        cached_tables_t.size() * sizeof(float));
            all_to_all_dists = new float[cached_all_to_all.size()];
            std::memcpy(all_to_all_dists, cached_all_to_all.data(),
                        cached_all_to_all.size() * sizeof(float));
            return;
          }
        }
      }
      ...
      post_load_pq_table();
      save_bin<float>(tables_t_cache_path, tables_T, ndims, NUM_PQ_CENTROIDS);
      save_bin<float>(all_to_all_cache_path, all_to_all_dists,
                      NUM_PQ_CENTROIDS, NUM_PQ_CENTROIDS * n_chunks);
    }
