...
struct IORequest {
  ...
  IORequest() : offset(0), len(0), buf(nullptr), finished(false), u_offset(0), u_len(0), base(nullptr) {
  }

  IORequest(uint64_t offset, uint64_t len, void *buf, uint64_t u_offset, uint64_t u_len, void *base = nullptr)
      : offset(offset), len(len), buf(buf), finished(false), u_offset(u_offset), u_len(u_len), base(base) {
