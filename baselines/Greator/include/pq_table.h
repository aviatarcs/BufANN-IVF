...
//_rearrangement_perm.bin
    ...
    if (file_exists(tables_t_cache_path) &&
        file_exists(all_to_all_cache_path)) {
      _u64 cache_nr = 0, cache_nc = 0;
      diskann::load_bin<float>(tables_t_cache_path, tables_T, cache_nr,
                               cache_nc);
      if (cache_nr == ndims && cache_nc == NUM_PQ_CENTROIDS) {
        ...
        if (cache_nr == NUM_PQ_CENTROIDS &&
          ...
          diskann::cout << "Loaded cached PQ runtime tables "
                        << tables_t_cache_path << std::endl;
          return;
        }
        delete[] all_to_all_dists;
