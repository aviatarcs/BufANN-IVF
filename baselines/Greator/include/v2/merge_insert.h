...
namespace diskann {
  ...
  class MergeInsert {
   public:
    ...
    // Query-only driver helper: use the update-capable loader, but preserve the
    // old disk-index-only search semantics when no updates are applied.
    DISKANN_DLLEXPORT void load_disk_cache();
    DISKANN_DLLEXPORT size_t search_disk_only(const T* query, const uint64_t K,
                                              const uint64_t search_L,
    ...
    DISKANN_DLLEXPORT size_t pending_delete_count();
   ...
   void save_del_set();
    void clear_deleted_tags_vector();
   ...
   private:
    ...
    RWlock _disk_lock;  // disk index switch
    ...
    ThreadPool* _search_tpool = nullptr;
