...
namespace diskann {
  ...
  DISKANN_DLLEXPORT unsigned env_u32(const char *name,
                                     unsigned    default_value);
  DISKANN_DLLEXPORT float env_f32(const char *name, float default_value);
  ...
  // Force single-threaded init of tsl empty-bucket statics (TBB warm-up is in
  // merge_insert.cpp). Safe to call multiple times — std::call_once inside.
  DISKANN_DLLEXPORT void init_greator_runtime();
}  // namespace diskann
