...
static int get_insert_threads() {
  return static_cast<int>(diskann::env_u32("INSERT_THREADS", NUM_INSERT_THREADS));
}

static unsigned get_merge_threshold() {
  return diskann::env_u32("MERGE_TH", MERGE_TH);
}

static int get_delete_threads() {
  return static_cast<int>(diskann::env_u32("DELETE_THREADS", NUM_DELETE_THREADS));
}

static int get_search_threads() {
  return static_cast<int>(diskann::env_u32("QUERY_THREADS", NUM_SEARCH_THREADS));
}
// read io info
...
#pragma omp parallel for num_threads(get_delete_threads())
  ...
  std::cout << "Mem index deletion time : " << timer.elapsed() / 1000 << " ms"
            << std::endl;
  if (npts > 0) {
    std::cout << "10th percentile deletion time : "
...
void update(const std::string& data_path, const unsigned L_mem,
  ...
  const unsigned    merge_threshold = get_merge_threshold();
  ...
  for (int i = 0; i < batch; i++) {
    ...
    if (inMmeorySize >= static_cast<int>(merge_threshold)) {
      get_io_info("begin");
...
void build(const std::string& data_path, const unsigned L_mem,
  ...
  paras.Set<unsigned>("R_disk", R_disk);
  paras.Set<float>("alpha_disk", alpha_disk);
  paras.Set<unsigned>("C", diskann::env_u32("C", 500));
