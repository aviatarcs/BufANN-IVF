...
namespace diskann {
  ...
  DiskNode<T>::DiskNode(uint32_t id, uint32_t nnbrs, uint32_t *nbrs) : id(id) {
    this->coords = nullptr;
    this->nnbrs = nnbrs;
    const size_t alloc_nbrs =
        std::max<size_t>(1, static_cast<size_t>(this->nnbrs));
    this->nbrs = new uint32_t[alloc_nbrs];
...
#pragma omp critical
  ...
  void PQFlashIndex<T, TagT>::destroy_thread_data() {
    while (!this->thread_data.empty()) {
      this->thread_data.pop();
    }
    for (auto &data : this->thread_data_backing_buf) {
      ...
      scratch.coord_scratch = nullptr;
      scratch.sector_scratch = nullptr;
    }
    this->thread_data_backing_buf.clear();
  ...
  void PQFlashIndex<T, TagT>::load_cache_list(
    ...
    node_list.erase(std::remove_if(node_list.begin(), node_list.end(),
                                   [this](uint32_t node_id) {
                                     return node_id >= this->num_points;
                                   }),
                    node_list.end());
    num_cached_nodes = node_list.size();
    ...
    for (_u64 block = 0; block < num_blocks; block++) {
      ...
      for (auto &nhood : nhoods) {
        ...
        auto      nnbrs = std::min<unsigned>(*node_nhood, max_degree);
        ...
        for (unsigned i = 0; i < nnbrs; i++) {
          if (nbrs[i] < this->num_points) {
            cnhood.second[valid_nnbrs++] = nbrs[i];
          }
        }
        cnhood.first = valid_nnbrs;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void PQFlashIndex<T, TagT>::cache_bfs_levels(
    ...
    for (_u64 miter = 0; miter < num_medoids; miter++) {
      if (medoids[miter] < this->num_points) {
...
// read index metadata
  ...
  void PQFlashIndex<T, TagT>::disk_iterate_to_fixed_point(
    ...
    // Start from every unique medoid, matching PipeANN's multi-entrypoint
    // search. Score entrypoints in the same PQ space as the rest of the
    // candidate list; if there are more entrypoints than L, retain the best L.
    compute_dists(medoids, num_medoids, dist_scratch);
    unsigned cur_list_size = 0;
    for (_u64 cur_m = 0; cur_m < num_medoids; cur_m++) {
      const _u32 medoid = medoids[cur_m];
      if (visited.find(medoid) != visited.end()) {
        continue;
      }
      visited.insert(medoid);

      Neighbor seed(medoid, dist_scratch[cur_m], true);
      if (cur_list_size == 0) {
        retset[0] = seed;
        cur_list_size = 1;
      } else {
        InsertIntoPool(retset.data(), cur_list_size, seed);
    ...
    while (k < cur_list_size && num_ios < IO_LIMIT) {
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        if (this->enable_tags &&
            this->tags[frontier_nhood.first] ==
                std::numeric_limits<uint32_t>::max()) {
          exclude_cur_node = true;
        }
  ...
  _u32 PQFlashIndex<T, TagT>::merge_read(std::vector<DiskNode<T>> &disk_nodes,
    ...
    pmutex->ReadLock();
    // big sequential read
    this->reader->sequential_read(read_req, ctx);
    pmutex->ReadUnLock();
    // create disk nodes
  ...
  void PQFlashIndex<T, TagT>::merge_write_through_diff_pages(
    ...
    size_t                   max_page_id = 0;
    for (size_t i = 0; i < start_id.size(); i++) {
      max_page_id = std::max(
          max_page_id,
          static_cast<size_t>(1 + start_id[i] / this->nnodes_per_sector));
    }
    if (max_page_id >= this->pagemutex.size()) {
      this->pagemutex.resize(max_page_id + 10);
    }
...
#pragma omp parallel for schedule(dynamic, chunk_size) num_threads(10)
  ...
  void PQFlashIndex<T, TagT>::merge_read_through_diff_pages(
    ...
    auto prepare_and_lock_read = [&](size_t i) {
      _u64 start_off = NODE_SECTOR_NO(((size_t) start_id[i])) * SECTOR_LEN;
...
#pragma omp parallel for schedule(dynamic, chunk_size) num_threads(10)
  ...
  void PQFlashIndex<T, TagT>::scan_deleted_nodes(
    ...
    if (!id_map) {
      ...
      while (n_scanned < this->num_points) {
        ...
        this->reader->sequential_read(reads, ctx);
  ...
  void PQFlashIndex<T, TagT>::reload_index(
    ...
    // reload_index reopens the on-disk index on this->reader. Per-thread state
    // (aligned scratch buffers + register_thread()/ctx wiring used by
    // disk_iterate_to_fixed_point and merge I/O) must match a fresh load().
    // On Linux, older code skipped teardown/re-setup here; that left scratch
    // and/or ThreadData copies (e.g. in StreamingMerger::disk_thread_data) out
    // of sync with the reopened reader session — not "io_context_t is bound to
    // one fd forever", but lifecycle: after reopen, redo the same init path
    // as after PQFlashIndex::load().
    if (this->load_flag) {
      this->destroy_thread_data();
    }
    ...
    this->setup_thread_data(this->max_nthreads);
  ...
  void PQFlashIndex<T, TagT>::get_active_tags(
    ...
    active_tags.clear();
    if (this->enable_tags) {
      active_tags.reserve(static_cast<size_t>(num_points));
      for (_u64 i = 0; i < num_points; i++) {
        if (this->num_frozen_points > 0) {
          if (i != frozen_location &&
              tags[i] != std::numeric_limits<uint32_t>::max())
            active_tags.insert(tags[i]);
        } else {
          if (tags[i] != std::numeric_limits<uint32_t>::max()) {
            active_tags.insert(tags[i]);
          }
        }
      }
