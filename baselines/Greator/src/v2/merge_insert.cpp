...
namespace diskann {
  ...
  MergeInsert<T, TagT>::MergeInsert(
    ...
    // Warm tsl static empty-bucket sentinels before any OMP/async workers start.
    diskann::init_greator_runtime();

    // Warm TBB allocator + runtime on the main thread.  merge_insert.cpp links
    // -ltbb; aux_utils.cpp does not, so the TBB touch lives here.
    // C++11 guarantees this static lambda runs exactly once, thread-safely.
    static const bool tbb_warmed = [] {
      tbb::concurrent_vector<uint32_t> v;
      v.push_back(0);
    ...
    _merge_th = parameters.Get<size_t>(
        "merge_th", static_cast<size_t>(env_u32("MERGE_TH", MERGE_TH)));
    _merge_nthreads = parameters.Get<uint64_t>(
        "merge_nthreads", static_cast<uint64_t>(env_u32("MERGE_THREADS", 1)));
    if (_merge_nthreads == 0) {
      _merge_nthreads = 1;
    }
  ...
  template<typename T, typename TagT>
  MergeInsert<T, TagT>::~MergeInsert() {
    clear_deleted_tags_vector();
    destruct_index_merger();
    delete _disk_index;
    _disk_index = nullptr;
    delete _search_tpool;
    _search_tpool = nullptr;
  }
  ...
  void MergeInsert<T, TagT>::construct_index_merger(uint32_t id_map) {
    ...
    if (id_map)
      ...
      _merger = new diskann::StreamingMerger<T, TagT>(
          (uint32_t) _dim, _dist_comp, _dist_metric, (uint32_t) _beamwidth,
          range, l_index, alpha, maxc, _single_file_index, 0,
          _merge_nthreads);
  ...
  template<typename T, typename TagT>
  void MergeInsert<T, TagT>::clear_deleted_tags_vector() {
    for (const auto* tags : _deleted_tags_vector) {
      delete tags;
    }
  ...
  template<typename T, typename TagT>
  void MergeInsert<T, TagT>::load_disk_cache() {
    std::vector<uint32_t> node_list;
    diskann::cout << "Caching " << _num_nodes_to_cache
  ...
  template<typename T, typename TagT>
  size_t MergeInsert<T, TagT>::search_disk_only(
      const T* query, const uint64_t K, const uint64_t search_L, TagT* tags,
      float* distances, QueryStats* stats) {
    std::shared_lock<RWlock> lock(_disk_lock);
    assert(_switching_disk_prefixes == false);
    return _disk_index->cached_beam_search(query, K, search_L, tags, distances,
                                           _beamwidth, stats);
  }

  template<typename T, typename TagT>
  void MergeInsert<T, TagT>::get_active_tags(
      tsl::robin_set<TagT>& active_tags) {
    std::shared_lock<RWlock> lock(_disk_lock);
    assert(_switching_disk_prefixes == false);
    _disk_index->get_active_tags(active_tags);
  }
  ...
  void MergeInsert<T, TagT>::final_merge(uint32_t id_map) {
    ...
    const size_t pending_deletes = pending_delete_count();
    if (_mem_points > 0 || pending_deletes > 0) {
      save_del_set();
      switch_index(id_map);
    }
    diskann::cout << _mem_index_0->get_num_points() << "  "
  ...
  void MergeInsert<T, TagT>::merge(uint32_t id_map) {
    ...
    {
      std::unique_lock<RWlock> lock(_disk_lock);
      bool                                      expected_value = false;
      const bool                                merged_in_place =
          (_disk_index_prefix_in == _disk_index_prefix_out);
  ...
  template<typename T, typename TagT>
  size_t MergeInsert<T, TagT>::pending_delete_count() {
    std::shared_lock<std::shared_timed_mutex> lock(_delete_lock);
    return _deletion_set_0.size() + _deletion_set_1.size();
  }
