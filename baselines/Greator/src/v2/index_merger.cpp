...
static uint64_t configured_merge_threads() {
  const char *value = std::getenv("MERGE_THREADS");
...
namespace diskann {
  ...
  StreamingMerger<T, TagT>::StreamingMerger(
      ...
      uint32_t id_map, uint64_t merge_nthreads) {
    // book keeping
    this->ndims = ndims;
    ...
    this->id_map = id_map;
    this->merge_nthreads =
        merge_nthreads == 0 ? configured_merge_threads() : merge_nthreads;
    if (this->merge_nthreads == 0) {
      this->merge_nthreads = 1;
    }
    ...
    std::cout << "StreamingMerger created with R=" << this->range
              ...
              << " ndims: " << this->ndims
              << " merge_nthreads: " << this->merge_nthreads << std::endl;
  }
  ...
  void StreamingMerger<T, TagT>::process_inserts_pq() {
    ...
    this->insert_times.resize(this->merge_nthreads, 0.0);
    this->delta_times.resize(this->merge_nthreads, 0.0);
    uint32_t n_inserts =
        std::accumulate(this->mem_npts.begin(), this->mem_npts.end(), 0);
    if (n_inserts == 0) {
      diskann::cout << "No inserts to deflate into PQ.\n";
      return;
    }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads((int) this->merge_nthreads)
  ...
  void StreamingMerger<T, TagT>::process_inserts() {
    ...
    const int worker_threads =
        static_cast<int>(std::max<uint64_t>(
            1, std::min<uint64_t>(this->merge_nthreads,
                                  static_cast<uint64_t>(
                                      this->disk_thread_data.size()))));
    this->insert_times.resize(static_cast<size_t>(worker_threads), 0.0);
    this->delta_times.resize(static_cast<size_t>(worker_threads), 0.0);
    uint32_t n_inserts =
        std::accumulate(this->mem_npts.begin(), this->mem_npts.end(), 0);
    if (n_inserts == 0) {
      diskann::cout << "No inserts to merge into the disk index.\n";
      return;
    }
    // iterate through each vector in each mem index
    for (uint32_t i = 0; i < this->mem_data.size(); i++) {
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(worker_threads)
  ...
  void StreamingMerger<T, TagT>::insert_mem_vec(const T       *mem_vec,
    ...
    if (!this->insert_times.empty()) {
      if (thread_no >= this->insert_times.size()) {
        thread_no = thread_no % static_cast<uint32_t>(this->insert_times.size());
      }
  ...
  void StreamingMerger<T, TagT>::offset_iterate_to_fixed_point(
    ...
    if (this->disk_thread_data.empty()) {
      throw ANNException("No disk thread contexts available for merge insert",
                         -1);
    }
    const uint32_t omp_thread_no = omp_get_thread_num();
    const size_t thread_data_index =
        static_cast<size_t>(omp_thread_no) % this->disk_thread_data.size();
    ThreadData<T> &thread_data = this->disk_thread_data[thread_data_index];
  ...
  void StreamingMerger<T, TagT>::dump_to_disk(const uint32_t start_id,
    ...
    // dump
    const uint64_t write_bytes = (uint64_t) n_sectors * (uint64_t) SECTOR_LEN;
    output_writer.write(buf, write_bytes);
    ann_bench::LogicalIoCounter::account_seq_write(
        write_bytes > 0 ? 1 : 0, write_bytes);
  ...
  void StreamingMerger<T, TagT>::process_deletes() {
    if (id_map == 2) {
      ...
      for (size_t j = 0; j < read_nums; ++j) {
        ...
        int chunk_size = 1 > (sector_ids.size() / (this->merge_nthreads))
                             ? 1
                             : (sector_ids.size() / (this->merge_nthreads));
#pragma omp parallel for schedule(dynamic, chunk_size) \
    num_threads((int) this->merge_nthreads)
        for (size_t i = 0; i < sector_ids.size(); i++) {
          start_ids[i] = (uint64_t) (sector_ids[i] - 1) * this->nnodes_per_sector;
        }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads((int) this->merge_nthreads)
  ...
  void StreamingMerger<T, TagT>::process_merges() {
    ...
    Timer  merge_timer;
    Timer  build_help_timer;
    const uint32_t old_disk_npts =
        static_cast<uint32_t>(this->disk_delta->graph.size());
    tbb::concurrent_unordered_map<uint32_t, tbb::concurrent_vector<uint32_t>>
        change_id_map;
    // disk
#pragma omp parallel for schedule(dynamic, 16) num_threads((int) this->merge_nthreads)
...
#pragma omp parallel for schedule(dynamic, chunk_size) \
    num_threads((int) this->merge_nthreads)
        ...
        std::vector<std::vector<DiskNode<T>>> disk_nodess(sector_ids.size());
        auto create_zero_backed_nodes = [&](size_t sector_pos) {
          uint32_t cur_node_id = start_ids[sector_pos];
          char    *sector_slot = buf + (sector_pos * SECTOR_LEN);
          memset(sector_slot, 0, SECTOR_LEN);
          auto &disk_nodes = disk_nodess[sector_pos];
          for (_u32 node_idx = 0;
               node_idx < this->nnodes_per_sector &&
               cur_node_id < this->disk_npts;
               node_idx++) {
            char *node_buf =
                sector_slot + ((cur_node_id % this->nnodes_per_sector) *
                               this->max_node_len);
            disk_nodes.emplace_back(
                cur_node_id, reinterpret_cast<T *>(node_buf),
                reinterpret_cast<uint32_t *>(node_buf + this->ndims * sizeof(T)));
            cur_node_id++;
          }
        };

        Timer io_r;
        std::vector<uint32_t> old_start_ids;
        std::vector<size_t>   old_positions;
        std::vector<char>     old_sector(start_ids.size(), 0);
        old_start_ids.reserve(sector_ids.size());
        old_positions.reserve(sector_ids.size());
        for (size_t i = 0; i < start_ids.size(); i++) {
          if (start_ids[i] < old_disk_npts) {
            old_start_ids.push_back(start_ids[i]);
            old_positions.push_back(i);
            old_sector[i] = 1;
          }
        }

        bool old_nodes_in_separate_buf = false;
        if (old_start_ids.size() == start_ids.size()) {
          this->disk_index->merge_read_through_diff_pages(
              disk_nodess, disk_tags, disk_deleted_ids, start_ids, buf, true);
        } else {
          if (!old_start_ids.empty()) {
            if (old_read_buf == nullptr) {
              alloc_aligned((void **) &old_read_buf,
                            (size_t) pages_per_read * SECTOR_LEN, SECTOR_LEN);
            }
            std::vector<std::vector<DiskNode<T>>> old_disk_nodess;
            this->disk_index->merge_read_through_diff_pages(
                old_disk_nodess, disk_tags, disk_deleted_ids, old_start_ids,
                old_read_buf, true);
            assert(old_disk_nodess.size() == old_positions.size());
            for (size_t i = 0; i < old_positions.size(); i++) {
              disk_nodess[old_positions[i]] = std::move(old_disk_nodess[i]);
            }
            old_nodes_in_separate_buf = true;
          }
          for (size_t i = 0; i < start_ids.size(); i++) {
            if (!old_sector[i]) {
              create_zero_backed_nodes(i);
            }
          }
        }
        io_time += (double) io_r.elapsed() / 1000000.0;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads((int) this->merge_nthreads)
        ...
        if (old_nodes_in_separate_buf) {
          for (size_t i = 0; i < old_positions.size(); i++) {
            memcpy(buf + (old_positions[i] * SECTOR_LEN),
                   old_read_buf + (i * SECTOR_LEN), SECTOR_LEN);
          }
        }
        Timer io_w;
        this->disk_index->merge_write_through_diff_pages(start_ids, buf);
      ...
      aligned_free((void *) buf);
      if (old_read_buf != nullptr) {
        aligned_free((void *) old_read_buf);
      }
    }
  ...
  void StreamingMerger<T, TagT>::merge(
...
#else
    this->reader = std::make_shared<BingAlignedFileReader>();
#endif

    //    std::shared_ptr<AlignedFileReader> reader =
    //    std::make_shared<MemAlignedFileReader>();
    this->disk_index = new PQFlashIndex<T, TagT>(
        this->dist_metric, this->reader, this->_single_file_index, true);
    ...
    this->disk_index->load(disk_in, static_cast<uint32_t>(this->merge_nthreads));
    ...
    alloc_aligned((void **) &this->thread_pq_scratch,
                  this->merge_nthreads * PER_THREAD_BUF_SIZE, SECTOR_LEN);
    this->thread_bufs.resize(this->merge_nthreads);
    ...
    mergeImpl(id_map);
    // A fresh StreamingMerger is built per merge round and getTempFilePath()
    // increments temp<i> while prior files exist -> temp indexes accumulate ~1
    // full disk.index/merge and fill the disk (ENOSPC) at billion scale. Drop
    // every temp*_temp_* in the temp dir except the one just made live. The
    // "_temp_disk_index" guard skips the in-place (id_map==2) "_disk.index"
    // path so the real index is never touched. See OVERNIGHT_NOTES.md.
    if (this->temp_disk_index_path.find("_temp_disk_index") != std::string::npos) {
      auto base_of = [](const std::string &p) {
        auto s = p.find_last_of('/');
        return s == std::string::npos ? p : p.substr(s + 1);
      };
      auto sp = this->temp_disk_index_path.find_last_of('/');
      const std::string dir = (sp == std::string::npos)
                                  ? std::string(".")
                                  : this->temp_disk_index_path.substr(0, sp);
      const std::string keep_d = base_of(this->temp_disk_index_path);
      const std::string keep_p = base_of(this->temp_pq_coords_path);
      const std::string keep_t = base_of(this->temp_tags_path);
      DIR *dp = opendir(dir.c_str());
      if (dp != nullptr) {
        struct dirent *ent;
        while ((ent = readdir(dp)) != nullptr) {
          const std::string nm = ent->d_name;
          const bool is_temp_artifact =
              nm.rfind("temp", 0) == 0 &&
              (nm.find("_temp_disk_index") != std::string::npos ||
               nm.find("_temp_pq_compressed") != std::string::npos ||
               nm.find("_temp_tags") != std::string::npos);
          if (is_temp_artifact && nm != keep_d && nm != keep_p && nm != keep_t) {
  ...
  void StreamingMerger<T, TagT>::mergeImpl(uint32_t id_map) {
    ...
    */
    ...
    uint32_t new_max_pts = this->disk_npts;
    if (!this->inverse_list.empty()) {
      new_max_pts = std::max(this->inverse_list.back().first + 1, new_max_pts);
    }
    std::cout << "old_disk_npts: " << this->disk_npts
              << " now_disk_npts: " << new_max_pts << "\n";
    if (this->disk_npts != new_max_pts) {
      _u8 *pq_old_data = this->pq_data;
      _u8 *pq_new_data = new _u8[(size_t) new_max_pts * this->pq_nchunks];
      memcpy(pq_new_data, pq_old_data,
             (size_t) this->disk_npts * this->pq_nchunks);
      delete[] pq_old_data;
    ...
    // re-acquire pointers; reload rebuilds PQFlashIndex thread scratch + IO ctx
    this->disk_thread_data = this->disk_index->get_thread_data();
    ...
    this->max_node_len = (_u32) this->disk_index->max_node_len;
    this->range =
        (this->max_node_len - (sizeof(T) * this->ndims)) / sizeof(uint32_t) - 1;
    this->init_ids.clear();
    ...
    auto validate_index_integrity = [](const std::string &index_file) {
      const uint64_t actual_size = ::get_file_size(index_file);
      if (actual_size == 0) {
        throw ANNException("Merged index integrity check failed: empty index file",
                           -1);
      }
      if (actual_size % SECTOR_LEN != 0) {
        throw ANNException(
            "Merged index integrity check failed: index file size is not 4K aligned",
            -1);
      }

      std::ifstream reader(index_file, std::ios::binary);
      if (!reader.is_open()) {
        throw ANNException(
            "Merged index integrity check failed: unable to open index file",
            -1);
      }

      uint32_t nr = 0, nc = 0;
      reader.read(reinterpret_cast<char *>(&nr), sizeof(uint32_t));
      reader.read(reinterpret_cast<char *>(&nc), sizeof(uint32_t));
      const uint64_t words = static_cast<uint64_t>(nr) * static_cast<uint64_t>(nc);
      if (!reader || words < 8) {
        throw ANNException(
            "Merged index integrity check failed: invalid metadata header shape",
            -1);
      }

      std::vector<uint64_t> metadata(words);
      reader.read(reinterpret_cast<char *>(metadata.data()),
                  static_cast<std::streamsize>(metadata.size() *
                                               sizeof(uint64_t)));
      if (!reader) {
        throw ANNException(
            "Merged index integrity check failed: unable to read metadata payload",
            -1);
      }

      const uint64_t npts = metadata[0];
      const uint64_t max_node_len = metadata[3];
      const uint64_t nnodes_per_sector = metadata[4];
      const uint64_t metadata_file_size = metadata[7];
      if (max_node_len == 0 || nnodes_per_sector == 0) {
        throw ANNException(
            "Merged index integrity check failed: invalid max_node_len/nnodes_per_sector",
            -1);
      }

      const uint64_t sector_count = actual_size / SECTOR_LEN;
      const uint64_t capacity_nodes =
          sector_count > 0 ? (sector_count - 1) * nnodes_per_sector : 0;
      if (capacity_nodes < npts) {
        throw ANNException(
            "Merged index integrity check failed: graph body cannot hold metadata npts",
            -1);
      }
      if (metadata_file_size > 0 && actual_size < metadata_file_size) {
        throw ANNException(
            "Merged index integrity check failed: actual file size is smaller than metadata file_size",
            -1);
      }

      std::cout << "Merged index integrity OK: file=" << index_file
                << " npts=" << npts << " actual_size=" << actual_size
                << " metadata_file_size=" << metadata_file_size
                << " capacity_nodes=" << capacity_nodes << std::endl;
    };
    validate_index_integrity(this->final_index_file);
  ...
  void StreamingMerger<T, TagT>::get_sector_with_deleted_nbrs_nodes(
          ...
          &sector_with_deleted_nbrs_nodes) {
    {
      ...
      ann_bench::LogicalIoCounter::account_seq_read(1, SECTOR_LEN);
      ...
      for (size_t i = 0; i < read_nums; i++) {
        ...
        const uint64_t bytes_to_read =
            static_cast<uint64_t>(SECTOR_LEN) * pages_num;
        infile.read(read_buf.data(), bytes_to_read);
        ann_bench::LogicalIoCounter::account_seq_read(1, bytes_to_read);
        ...
        int chunk_size = 1 > (pages_num / (this->merge_nthreads))
                             ? 1
                             : (pages_num / (this->merge_nthreads));
...
#pragma omp parallel for schedule(dynamic, chunk_size) \
  ...
  template<typename T, typename TagT>
  void StreamingMerger<T, TagT>::update_topology_header_npts(
      const std::string &path, _u64 npts) {
    int fd = open(path.c_str(), O_WRONLY);
    if (fd < 0) {
      throw ANNException("Unable to open topology sidecar header for update: " +
                             path,
                         -1);
    }

    ssize_t written = pwrite(fd, reinterpret_cast<const char *>(&npts),
                             sizeof(_u64), 0);
    close(fd);
    if (written != static_cast<ssize_t>(sizeof(_u64))) {
      throw ANNException("Unable to update topology sidecar npts header: " +
                             path,
                         -1);
    }

    std::cout << "Updated topology sidecar header npts: " << path << " -> "
              << npts << std::endl;
  }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads((int) this->merge_nthreads)
      for (size_t i = 0; i < delete_nodes.size(); i++) {
        ...
        if (node.nnbrs == 0) {
          ...
          ssize_t written = pwrite(fd, (char *) &node.nnbrs, sizeof(int), off);
          if (written > 0) {
            ann_bench::LogicalIoCounter::account_random_write(
                1, static_cast<uint64_t>(written));
          }
        } else {
          offset_t off =
              (1 + (offset_t) node.id / this->topological_nnodes_per_sector) *
      ...
      close(fd);
      update_topology_header_npts(outdir, static_cast<_u64>(this->disk_npts));
  ...
  void StreamingMerger<T, TagT>::process_topological_merge() {
    {
      ...
      std::vector<char> data_buf(merge_nodes.size() * (this->range + 1) *
                                 sizeof(int));
      std::cout << "Start process_topological_merge for "
                << merge_nodes.size() << " nodes using "
                << this->merge_nthreads << " threads" << std::endl;
      // NOTE: This uses buffered write on a tiny granularity (a few hundred bytes). This means the kernel need to read a missing page into buffer cache, and this whole process is protected by an exclusive lock. This will be even slower than mmap, cuz mmap can fault many pages in parallel while buffered write serializes to load one page at a time.
      // The speed is simply abysmal
      // NOTE: If one wanted to use mmap here: need to make sure not to write outside of mmap'ed region.
#pragma omp parallel for schedule(dynamic, 16) num_threads((int) this->merge_nthreads)
