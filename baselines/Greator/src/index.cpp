...
#pragma omp parallel for schedule(dynamic)
  ...
  int Index<T, TagT>::insert_point(const T *point, const Parameters &parameters,
    ...
    {
      // Outer function already holds shared _update_lock; do not lock_shared
      // _update_lock again here (same thread) — that is undefined behavior and
      // can corrupt the heap after many inserts.
      // std::shared_lock<std::shared_timed_mutex> lock(_update_lock);
