...
namespace diskann {
  ...
  float env_f32(const char *name, float default_value) {
    const char *value = std::getenv(name);
    if (value == nullptr || *value == '\0') {
      return default_value;
    }
  ...
  double calculate_recall(unsigned num_queries, unsigned *gold_std,
    ...
    const double recall_percent =
        total_recall / (num_queries) * (100.0 / recall_at);
    std::cout << "Recall@" << recall_at << ": " << recall_percent << " %"
              << std::endl;
  ...
  int build_merged_vamana_index(
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      paras.Set<unsigned>("C", env_u32("C", 750));
      paras.Set<float>("alpha", env_f32("ALPHA_DISK", env_f32("ALPHA", 1.2f)));
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  bool build_disk_index(const char *dataPath, const char *indexFilePath,
    ...
    if (param_list.size() != 5 && param_list.size() != 6) {
      diskann::cout
          << "Correct usage of parameters is: R (max degree)"
             ...
             " [num_pq_chunks (overrides parameter B for PQ chunk count)] "
          << std::endl;
      return false;
    }
    ...
    if (param_list.size() == 6) {
      num_pq_chunks = static_cast<size_t>(std::stoul(param_list[5]));
      if (num_pq_chunks == 0) {
        diskann::cerr << "Configured num_pq_chunks must be > 0." << std::endl;
        return false;
      }
      num_pq_chunks = std::min(num_pq_chunks, static_cast<size_t>(dim));
      num_pq_chunks = std::min(num_pq_chunks, static_cast<size_t>(MAX_PQ_CHUNKS));
      diskann::cout << "Using configured num_pq_chunks: " << num_pq_chunks
                    << std::endl;
    } else {
      num_pq_chunks =
  ...
  template DISKANN_DLLEXPORT int build_merged_vamana_index<uint8_t>(
      ...
      std::string centroids_file, const char *tag_file);
  // ---------------------------------------------------------------------------
  // init_greator_runtime
  // ---------------------------------------------------------------------------
  // tsl::robin_{set,map} initializes a type-specific empty-bucket static on
  // the very first construction of each instantiation.  When multiple OMP or
  // std::async threads call cached_beam_search (or the merger) simultaneously
  // on cold code, they race through __cxa_atexit inside that constructor.
  // The fix: touch every relevant tsl type once on the main thread, before any
  // worker is spawned.  std::call_once makes it idempotent and safe to call
  // from library entry points (MergeInsert ctor, etc.).
  //
  // TBB runtime warm-up lives in merge_insert.cpp (not here) so aux_utils.o
  // stays TBB-free and targets like build_disk_index that link libdiskann
  // without -ltbb still build.
  void init_greator_runtime() {
    static std::once_flag once;
    std::call_once(once, []() {
      { tsl::robin_set<unsigned>                        s; (void)s; }
      { tsl::robin_set<uint32_t>                        s; (void)s; }
      { tsl::robin_set<uint64_t>                        s; (void)s; }
      { tsl::robin_map<uint32_t, uint32_t>              m; (void)m; }
      { tsl::robin_map<uint32_t, float *>               m; (void)m; }
      { tsl::robin_map<uint32_t, uint8_t *>             m; (void)m; }
      { tsl::robin_map<uint32_t, int8_t *>              m; (void)m; }
      { tsl::robin_map<uint32_t, std::vector<uint32_t>> m; (void)m; }
    });
  }
};  // namespace diskann
