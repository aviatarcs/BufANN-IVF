...
namespace diskann {
  ...
  inline void get_bin_metadata_impl(std::basic_istream<char>& reader,
    ...
    if (nrows_32 < 0 || ncols_32 < 0) {
      throw diskann::ANNException(
          "Invalid binary file header: negative nrows or ncols", -1,
  ...
  inline void get_bin_metadata(MemoryMappedFiles& files,
    ...
    if (nrows_32 < 0 || ncols_32 < 0) {
      throw diskann::ANNException(
          "Invalid binary file header: negative nrows or ncols in " + bin_file,
  ...
  inline void load_bin_impl(std::basic_istream<char>& reader, T*& data,
    ...
    if (npts_i32 < 0 || dim_i32 < 0) {
      throw diskann::ANNException(
          "Invalid binary file header: negative npts or dim", -1, __FUNCSIG__,
          __FILE__, __LINE__);
    }
    ...
    data = new T[npts * dim];
    reader.read((char*) data, static_cast<std::streamsize>(npts * dim * sizeof(T)));
  }
  ...
  inline void load_truthset(const std::string& bin_file, uint32_t*& ids,
    ...
    if (npts_i32 < 0 || dim_i32 < 0) {
      throw diskann::ANNException(
          "Invalid truthset file header: negative npts or dim in " + bin_file,
          -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    npts = static_cast<size_t>(npts_i32);
    dim = static_cast<size_t>(dim_i32);
  ...
  inline void load_aligned_bin_impl(std::basic_istream<char>& reader, T*& data,
    ...
    if (npts_i32 < 0 || dim_i32 < 0) {
      throw diskann::ANNException(
          "Invalid aligned binary file header: negative npts or dim", -1,
          __FUNCSIG__, __FILE__, __LINE__);
    }
    npts = static_cast<size_t>(npts_i32);
    dim = static_cast<size_t>(dim_i32);
...
#pragma omp parallel for schedule(static, 65536)
  ...
  inline void copy_aligned_data_from_file(const std::string bin_file, T*& data,
    ...
    if (npts_i32 < 0 || dim_i32 < 0) {
      throw diskann::ANNException(
          "Invalid binary file header: negative npts or dim in " + bin_file,
          -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    npts = static_cast<size_t>(npts_i32);
    dim = static_cast<size_t>(dim_i32);
