...
void dump_to_disk(const T *all_pts, const uint64_t ndims,
  ...
  diskann::cout << "# points : " << tags.size() << std::endl;
  diskann::cout << "Tag file : " << tag_filename << std::endl;
  diskann::cout << "Data file : " << data_filename << std::endl;
...
void run(const uint32_t base_count, const uint32_t num_mem_indices,
  ...
  diskann::cout << "Loaded " << npts << " pts x " << ndims << " dims" << std::endl;
  //  assert(npts >= base_count + num_mem_indices);

  std::vector<uint32_t> tags(npts);
  std::iota(tags.begin(), tags.end(), 0);

  diskann::cout << "Base Index : choosing " << base_count << " points" << std::endl;
  ...
  diskann::cout << "Dumping base set " << std::endl;
  // write base
  ...
  while (count++ < num_cycles) {
    ...
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      ...
      diskann::cout << "Index #" << i + 1 << " : choosing "
                    << new_start - cur_start << " points" << std::endl;
    ...
    // write mem
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      diskann::cout << "Dumping mem set #" << i + 1 << std::endl;
    ...
    // write mem
    //  diskann::cout << "Dumping {all} \\ {deleted} set" << std::endl;
...
int main(int argc, char **argv) {
  ...
  if (index_type == std::string("float")) {
    ...
    diskann::cout << "Unsupported type : " << index_type << std::endl;
  }
  diskann::cout << "Exiting" << std::endl;
}
