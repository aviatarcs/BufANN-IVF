...
inline int get_num_parts(const char *filename) {
  ...
  diskann::cout << "Reading bin file " << filename << " ..." << std::endl;
...
inline void save_bin(const std::string filename, T *data, size_t npts,
  ...
  diskann::cout << "Writing bin: " << filename << std::endl;
...
int aux_main(int argc, char **argv) {
  ...
  uint32_t *tags = nullptr;
  if (argc == 7) {
    std::cout << "Loading tags from " << argv[6] << std::endl;
    ...
    diskann::cout << "Loaded tags for " << tag_pts << " points." << std::endl;
