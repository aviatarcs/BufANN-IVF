...
namespace diskann {
  ...
  PQFlashIndex<T, TagT>::~PQFlashIndex() {
    ...
    diskann::cout << "Thread Data size: " << this->thread_data.size() << std::endl;
  ...
  void PQFlashIndex<T, TagT>::load_cache_list(
    ...
    for (_u64 block = 0; block < num_blocks; block++) {
      ...
      for (auto &nhood : nhoods) {
        ...
        // diskann::cout << "CACHE: nnbrs = " << nnbrs << std::endl;
...
// read index metadata
  ...
  void PQFlashIndex<T, TagT>::reload_index(
    ...
    diskann::cout << "RELOAD: Loading compressed vectors from "
                  << pq_compressed_vectors << std::endl;
    ...
    diskann::cout << "RELOAD: Loading graph from " << disk_index_file << std::endl;
    ...
    diskann::cout << "Medoid-ID: " << medoid_id_on_file << std::endl;
    ...
    // clear cache
    diskann::cout << "RELOAD: Clearing cache." << std::endl;
    ...
    // refresh cache
    diskann::cout << "RELOAD: Caching " << node_cache_count << " nodes." << std::endl;
    std::vector<uint32_t> node_list;
