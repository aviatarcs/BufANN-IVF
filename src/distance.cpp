...
#pragma omp simd reduction(+ : result) aligned(a, b : 8)
  ...
  float DistanceL2::compare(const float *a, const float *b,
    ...
    float result = 0;
#ifdef __AVX512F__
    // Accept unaligned inputs in the AVX-512 path.
    const uint16_t niters16 = (uint16_t)(size / 16);
    __m512 sum16 = _mm512_setzero_ps();
    for (uint16_t j = 0; j < niters16; j++) {
      if (j < (niters16 - 1)) {
      ...
      __m512 tmp   = _mm512_sub_ps(a_vec, b_vec);
      sum16 = _mm512_fmadd_ps(tmp, tmp, sum16);
    }
    result = _mm512_reduce_add_ps(sum16);
    // Handle 8-float tail (when aligned_dim % 16 == 8).
    const uint16_t rem = niters16 * 16;
    if (rem < size) {
      __m256 a_rem = _mm256_loadu_ps(a + rem);
      __m256 b_rem = _mm256_loadu_ps(b + rem);
      __m256 tmp   = _mm256_sub_ps(a_rem, b_rem);
      result += _mm256_reduce_add_ps(_mm256_fmadd_ps(tmp, tmp, _mm256_setzero_ps()));
    }
